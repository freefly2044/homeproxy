...
function renderNodeSettings(section, data, features, main_node, routing_mode) {
	...
	o.depends({ 'type': 'selector', '!reverse': true });
	o.rmempty = false;
	...
	o = s.option(form.MultiValue, 'selector_outbounds', _('Outbounds'));
	o.depends('type', 'selector');
	o.load = function (section_id) {
		let nodes = [];
		uci.sections(data[0], 'routing_node', (res) => {
			if (res['.name'] !== section_id && res.type !== 'selector') {
				nodes.push({
					name: res['.name'],
		...
		for (let i of nodes)
			this.value(i.name, i.label);
	}
	o.modalonly = true;

	o = s.option(form.ListValue, 'selector_default', _('Default'));
	o.depends('type', 'selector');
	o.load = function (section_id) {
		let nodes = [];
		uci.sections(data[0], 'routing_node', (res) => {
